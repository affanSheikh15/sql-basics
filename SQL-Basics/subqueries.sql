create database if not exists subquerdb;
use subquerdb;
select database();
...
select category,sum(amount)
 ...
 -- view virtual table create krta 
 
 create view categorysales as
 select category,sum(amount)as totalsale
 from sales
 group by category;
 
 select * from categorysales where totalsale > 500 ;
